				...
				try {
					for (var _iterator = this.hubs[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
						(function () {
							...
							_this.io.of(Hub.namespace()).on("connection", function (socket) {
								_this.onConnection(Hub.namespace());
								new Hub(Hub.namespace(), _this.io, socket);
								socket.on("disconnect", _this.onDisconnect.bind(_this, Hub.namespace()));
							});
						})();
					}
				} catch (err) {
